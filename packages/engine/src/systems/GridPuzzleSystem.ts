...
export class GridPuzzleSystem extends ReactiveSystem {
  ...
  onInit({scene, events}: Omit<SystemContext, 'deltaTime'>): void {
    ...
    if (config.generator && (!puzzle.given || puzzle.given.length === 0)) {
      const fn = puzzleGenerators.get(config.generator);
      if (fn) {
        const generated = fn(config);
    ...
    const prefix = config.cellPrefix ?? 'cell';
    ...
    for (let row = 0; row < rows; row++) {
      ...
      for (let col = 0; col < cols; col++) {
        const bg = scene.getEntity(`${prefix}-${row}-${col}`)?.getComponent<RenderableComponent>('Renderable');
        const num = scene.getEntity(`${prefix}-${row}-${col}-num`)?.getComponent<RenderableComponent>('Renderable');
